...
const getAllPosts = (request, reply) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
const createPost = (request, reply) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const ID = (0, jwt_decode_1.default)(token !== null && token !== void 0 ? token : '').sub;
        if (!ID) {
            reply.send("Token is not valid. Please, relogin.");
        }
        const post = yield posts_1.Posts.create({ title, content, category, tags, _creator: ID });
...
const updatePost = (request, reply) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const token = (_b = request.headers["authorization"]) === null || _b === void 0 ? void 0 : _b.replace('Bearer ', '');
        const ID = (0, jwt_decode_1.default)(token !== null && token !== void 0 ? token : '').sub;
        if (!ID) {
            reply.send("Token is not valid. Please, relogin.");
        }
        var creator = postToFind === null || postToFind === void 0 ? void 0 : postToFind.creator.valueOf();
        if (creator !== ID) {
            reply.send("This post is not available to edit for this user");
        }
        const { title, content, category, tags } = request.body;
        const postToUpdate = yield posts_1.Posts.findByIdAndUpdate(id, { title, content, category, tags }, { new: true }).exec();
        return postToUpdate;
    }
    catch (error) {
        console.error(error);
...
const deletePost = (request, reply) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const token = (_c = request.headers["authorization"]) === null || _c === void 0 ? void 0 : _c.replace('Bearer ', '');
        const ID = (0, jwt_decode_1.default)(token !== null && token !== void 0 ? token : '').sub;
        if (!ID) {
            reply.send("Token is not valid. Please, relogin.");
        }
        var creator = postToFind === null || postToFind === void 0 ? void 0 : postToFind.creator.valueOf();
        if (creator !== ID) {
            reply.send("This post is not available to delete for this user");
        }
        const postToDelete = yield posts_1.Posts.findByIdAndDelete(id).exec();
        if (!postToDelete)
            reply.code(404).send('Post was not found');
        return postToDelete;
    }
